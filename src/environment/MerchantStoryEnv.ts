...
import { MERCHANT_BRANCH_GRAPH } from "../model/BranchGraph";

export class MerchantStoryEnv {
  private maxSteps: number;
  private currentState: MerchantStoryState;
  ...
  constructor(
    private userFeatureDim: number,
    private maxStepsPerEpisode: number
  ) {
  ...
  public reset(): MerchantStoryState {
    this.stepsCount = 0;
    this.currentState = {
  ...
  public step(action: number): {
    ...
    const node = MERCHANT_BRANCH_GRAPH.find(n => n.id === this.currentState.branchId);
    if (!node) {
    ...
    const newUserFeatures = this.currentState.userFeatures.map(
      f => f + (Math.random() * 0.01)
    );
  ...
  private getState(): MerchantStoryState {
    // return a copy
